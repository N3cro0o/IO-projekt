...
const CheckResults: React.FC = () => {
    ...
    const userId = 1; // (na sztywno) Ustawienie ID u¿ytkownika - DO POPRAWY
                    ...
                    {results.map((result, index) => (
                        <ListItem component="div" key={index} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'start', mb: 2 }}>
                            <ListItemText
                                ...
                                secondary={`Category: ${result.category} | Points: ${result.points} | Feedback: ${result.feedback || 'No feedback'}`}
                            />
                        </ListItem>
                    ))}
                </List>
